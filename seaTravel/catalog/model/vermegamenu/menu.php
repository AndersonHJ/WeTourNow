...
//        $html[] = '</a>';
  ...
  public function drawMenuItem($children, $level = 1, $columChunk = 0, $lang_id = 1) {
	...
	//	echo '--'; echo "<pre>";print_r($resultParents[0]);  echo "</pre>";
		...
		if($id_category !=0) $link = $this->url->link('product/category', "path=".$resultIds[$id_category]['category_id']);
		if($id_category !=0) $des = $resultIds[$id_category]['description'];
		if($id_category !=0) $thumbnail = $this->model_tool_image->resize($resultIds[$id_category]['thumbnail_image'], 20,20);
        $return = array(
					'id' => $id_category, 
					'link' => $link,
		 			'name' => $name, 
